...
COLOR_TRIGGER_HEADERS = [
    "US Color Additive Trigger",
    "US Color Trigger Basis",
    "US Color Match Status",
    "US Color Category",
    "US Color Matched Name",
    "US Color CAS",
    "US Color Uses and Restrictions",
    "US Color Regulatory Notes",
]
CI_NUMBER_PATTERN = re.compile(r"\bCI\s*[-:]?\s*(\d{4,6})\b", re.IGNORECASE)
COLOR_NAME_NUMBER_PATTERN = re.compile(
    r"\b(?:red|yellow|blue|green|violet|orange|black|brown)\s*(?:no\.?\s*)?\d+\b",
    re.IGNORECASE,
)
...
def normalized_upper_text(*vals: object) -> str:
    parts = [str(v).strip().upper() for v in vals if v is not None and str(v).strip()]
    return " ".join(parts)


def extract_ci_numbers(text: object) -> List[str]:
    if text is None:
        return []
    vals: List[str] = []
    for match in CI_NUMBER_PATTERN.findall(str(text)):
        if match not in vals:
            vals.append(match)
    return vals


def color_name_variants(text: object) -> List[str]:
    variants = exact_name_keys(text)
    raw = str(text or "").strip()
    upper = raw.upper()
    for keyword in COLOR_ADDITIVE_KEYWORDS:
        if keyword in upper:
            standardized = upper
            standardized = standardized.replace("FD & C", "FD&C")
            standardized = standardized.replace("D & C", "D&C")
            standardized = standardized.replace("EXT D&C", "EXT. D&C")
            standardized = re.sub(r"\([^)]*\)", " ", standardized)
            standardized = re.sub(r"\b(RED|YELLOW|BLUE|GREEN|VIOLET|ORANGE|BLACK|BROWN)\s+(\d+)\b", r"\1 NO. \2", standardized)
            nk = normalize_name_key(standardized)
            if nk and nk not in variants:
                variants.append(nk)
    return variants


def detect_color_trigger(raw_name: object, normalized_name: object = None) -> Tuple[bool, List[str], List[str]]:
    text = normalized_upper_text(raw_name, normalized_name)
    reasons: List[str] = []
    ci_numbers = extract_ci_numbers(text)
    if ci_numbers:
        reasons.append("CI number detected")
    if any(keyword in text for keyword in COLOR_ADDITIVE_KEYWORDS):
        reasons.append("color additive keyword detected")
    if COLOR_NAME_NUMBER_PATTERN.search(text):
        reasons.append("color name/number pattern detected")
    deduped = []
    for reason in reasons:
        if reason not in deduped:
            deduped.append(reason)
    return bool(deduped), deduped, ci_numbers
...
def load_us_color(path: Path) -> pd.DataFrame:
    sheet_category_map = {
        "73_SubpartC_Allowed": "73 Subpart C - Exempt from certification",
        "74_SubpartC_Certified": "74 Subpart C - Certification required",
        "Not_For_Cosmetics": "Not listed for current cosmetic use",
    }
    ...
    for sheet_name, category in sheet_category_map.items():
        if sheet_name not in xls.sheet_names:
            continue
        df = pd.read_excel(path, sheet_name=sheet_name).rename(columns=lambda x: str(x).strip())
        if df.empty or "Name" not in df.columns:
            continue
        use_col = next((c for c in df.columns if "Uses and restrictions" in str(c)), None)
        cas_col = next((c for c in df.columns if "CAS" in str(c)), None)
        part81_col = next((c for c in df.columns if "Part 81" in str(c)), None)
        part82_col = next((c for c in df.columns if "Part 82" in str(c)), None)
        out = pd.DataFrame()
        out["ingredient_name"] = df["Name"].fillna("").astype(str).str.strip()
        out = out[out["ingredient_name"] != ""].copy()
        out["db_source"] = "US_COLOR"
        out["color_category"] = category
        out["uses_and_restrictions"] = "" if use_col is None else df[use_col].fillna("").astype(str).str.strip()
        out["part81_notes"] = "" if part81_col is None else df[part81_col].fillna("").astype(str).str.strip()
        out["part82_notes"] = "" if part82_col is None else df[part82_col].fillna("").astype(str).str.strip()
        out["cas_raw"] = "" if cas_col is None else df[cas_col].fillna("").astype(str).str.strip()
        out["cas_list"] = out["cas_raw"].apply(lambda x: [normalize_cas(v) for v in split_multi_cas_value(x) if normalize_cas(v)])
        out["name_key"] = out["ingredient_name"].apply(normalize_name_key)
        out["name_variants"] = out["ingredient_name"].apply(color_name_variants)
        rows.append(out[["db_source", "ingredient_name", "name_key", "name_variants", "cas_list", "color_category", "uses_and_restrictions", "part81_notes", "part82_notes"]])
    if not rows:
        return pd.DataFrame(columns=["db_source", "ingredient_name", "name_key", "name_variants", "cas_list", "color_category", "uses_and_restrictions", "part81_notes", "part82_notes"])
...
def build_db_bundle(file_map: Dict[str, Path]) -> Dict[str, pd.DataFrame]:
    return {
        ...
        "us_color": load_us_color(file_map["us_color"]),
    }
...
def join_unique_field(rows: List[dict], field_name: str, sep: str = " | ") -> str:
    vals = []
    for row in rows:
...
def build_us_color_notes(rows: List[dict]) -> str:
    bits = []
    part81 = join_unique_field(rows, "part81_notes", sep=" | ")
    part82 = join_unique_field(rows, "part82_notes", sep=" | ")
    if part81:
        bits.append(f"Historical Part 81 references: {part81}")
    if part82:
        bits.append(f"Part 82 classification: {part82}")
    return " ; ".join(bits)
...
def match_one_record(rec: dict, db_bundle: Dict[str, pd.DataFrame], indexes: Dict[str, dict], candidate_pool: List[dict], ai: AIConfig, fuzzy_log_rows: List[dict]) -> dict:
    ...
    color_name_keys = color_name_variants(raw_name)
    color_triggered, color_trigger_reasons, ci_numbers = detect_color_trigger(raw_name, normalized_name)
    color_cas_trigger = any(indexes["us_color"]["cas"].get(cas) for cas in raw_cas_keys)
    if color_cas_trigger and "CAS matched US color database" not in color_trigger_reasons:
        color_trigger_reasons.append("CAS matched US color database")
        color_triggered = True
    color_hit_rows = lookup_hits("us_color", color_name_keys, raw_cas_keys, db_bundle, indexes, include_synonyms=False)
    color_has_not_for_cosmetics = any(
        str(r.get("color_category", "")).strip() == "Not listed for current cosmetic use"
        for r in color_hit_rows
    )
    if color_hit_rows:
        color_triggered = True
        out["US Color Additive Trigger"] = "Y"
        out["US Color Trigger Basis"] = " / ".join(color_trigger_reasons) if color_trigger_reasons else "exact name/CAS hit in US color database"
        out["US Color Match Status"] = "MATCHED"
        out["US Color Category"] = join_unique_field(color_hit_rows, "color_category")
        out["US Color Matched Name"] = join_hit_ingredients(color_hit_rows)
        out["US Color CAS"] = join_hit_cas(color_hit_rows)
        out["US Color Uses and Restrictions"] = join_unique_field(color_hit_rows, "uses_and_restrictions", sep="\n---\n")
        out["US Color Regulatory Notes"] = build_us_color_notes(color_hit_rows)
        manual_review = "Y"
        reviewer_notes.append("US color additive database matched. Manual review required for color additive workflow.")
        if ci_numbers:
            reviewer_notes.append(f"CI reference detected in input: {'; '.join(ci_numbers)}.")
        if color_has_not_for_cosmetics:
            reviewer_notes.append("US color additive database indicates current cosmetic use is not supported for the matched color additive.")
    elif color_triggered:
        out["US Color Additive Trigger"] = "Y"
        out["US Color Trigger Basis"] = " / ".join(color_trigger_reasons)
        out["US Color Match Status"] = "TRIGGER_ONLY"
        out["US Color Category"] = "Manual mapping needed"
        if ci_numbers:
            out["US Color Regulatory Notes"] = f"CI reference detected in input: {'; '.join(ci_numbers)}. Current US color database does not support CI-number direct mapping."
            reviewer_notes.append(f"Color additive trigger detected from CI reference {'; '.join(ci_numbers)}. Database mapping pending.")
        else:
            reviewer_notes.append("Color additive trigger detected from name pattern/keyword, but no exact US color database row matched.")
        manual_review = "Y"
    ...
    elif out["US Color Additive Trigger"] == "Y":
        out["Hard Stop?"] = "REVIEW"
        if color_has_not_for_cosmetics:
            out["Regulatory Summary"] = "Color additive matched the US color database, but current cosmetic use is not listed."
            out["Recommended Action"] = "Escalate for regulatory review and verify whether the color additive is permitted for the intended cosmetic use."
        elif out["US Color Match Status"] == "MATCHED":
            out["Regulatory Summary"] = "Color additive matched the US color database. Manual review is required for the color additive pathway."
            out["Recommended Action"] = "Review the matched color additive category and uses/restrictions before final release."
        else:
            out["Regulatory Summary"] = "Color additive trigger detected, but database mapping is not yet confirmed."
            out["Recommended Action"] = "Resolve the color additive identity manually before final conclusion."
...
def ensure_output_sheet_color_section(ws) -> None:
    from openpyxl.utils import get_column_letter

    existing_headers = [str(ws.cell(2, c).value).strip() for c in range(1, ws.max_column + 1)]
    if COLOR_TRIGGER_HEADERS[0] in existing_headers:
        return

    start_col = ws.max_column + 1
    group_src = ws.cell(1, 31)
    header_src = ws.cell(2, 31)
    body_src = ws.cell(3, 31)
    widths = [14, 22, 16, 28, 28, 18, 45, 28]

    ws.merge_cells(start_row=1, start_column=start_col, end_row=1, end_column=start_col + len(COLOR_TRIGGER_HEADERS) - 1)
    group_cell = ws.cell(1, start_col)
    group_cell.value = "US Color Additives"
    group_cell._style = copy(group_src._style)

    for offset, header in enumerate(COLOR_TRIGGER_HEADERS):
        cell = ws.cell(2, start_col + offset)
        cell.value = header
        cell._style = copy(header_src._style)
        body_cell = ws.cell(3, start_col + offset)
        body_cell._style = copy(body_src._style)
        ws.column_dimensions[get_column_letter(start_col + offset)].width = widths[offset]
...
def build_output_workbook(template_path: Path, output_path: Path, client_df: pd.DataFrame, output_df: pd.DataFrame, summary_df: pd.DataFrame, fuzzy_df: pd.DataFrame) -> None:
    ...
    ensure_output_sheet_color_section(ws03)
